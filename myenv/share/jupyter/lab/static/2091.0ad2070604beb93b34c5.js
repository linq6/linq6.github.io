    ...
    62091: (O, Q, e) => {
      ...
      const h = new a.Jq(
        (O, Q) => {
          ...
          if (e != c && e != X && e != -1 && !Q.context && Q.canShift($)) O.acceptToken($);
        },
        { contextual: true }
      );
      const W = new a.Jq(
      ...
      const x = a.WQ.deserialize({
        ...
        tokenizers: [
          ...
          new a.RA("j~RQYZXz{^~^O'p~~aP!P!Qd~iO'q~~", 25, 307),
        ],
      ...
      const k = [
        ...
        (0, w.Gn)('import {${names}} from "${module}"\n${}', { label: "import", detail: "named", type: "keyword" }),
        (0, w.Gn)('import ${name} from "${module}"\n${}', { label: "import", detail: "default", type: "keyword" }),
      ];
      const _ = k.concat([
        (0, w.Gn)("interface ${name} {\n\t${}\n}", { label: "interface", detail: "definition", type: "keyword" }),
        (0, w.Gn)("type ${name} = ${type}", { label: "type", detail: "definition", type: "keyword" }),
      ...
      function z(O, Q) {
        ...
        Q.cursor(V.IterMode.IncludeAnonymous).iterate((Q) => {
          if (i) {
            ...
            let O = C[Q.name];
            if ((O && O(Q, $)) || q.has(Q.name)) return false;
      ...
      function J(O, Q, e) {
        ...
        for (;;) {
          let $ = Q.firstChild,
            t;
          if (($ === null || $ === void 0 ? void 0 : $.name) == "VariableName") {
            i.push(O($));
            return { path: i.reverse(), name: e };
          } else if (
            ($ === null || $ === void 0 ? void 0 : $.name) == "MemberExpression" &&
      ...
      function N(O, Q) {
        ...
        for (let $ = 0; ; $++) {
          for (let r of (Object.getOwnPropertyNames || Object.keys)(O)) {
            ...
            e.push({
              label: r,
              type: typeof t == "function" ? (/^[A-Z]/.test(r) ? "class" : Q ? "function" : "method") : Q ? "variable" : "property",
              boost: -$,
            });
          }
      ...
      function D(O) {
        ...
        return (e) => {
          ...
          let $ = Q.get(i);
          if (!$) Q.set(i, ($ = N(i, !a.path.length)));
          return { from: e.pos - a.name.length, options: $, validFor: I };
        };
      }
      ...
      function aO(O = {}) {
        ...
        return new d.LanguageSupport(Q, [B.data.of({ autocomplete: (0, w.eC)(E, (0, w.Mb)(e)) }), B.data.of({ autocomplete: A }), O.jsx ? rO : []]);
      }
      function iO(O) {
        for (;;) {
      ...
      const tO = typeof navigator == "object" && /Android\b/.test(navigator.userAgent);
      const rO = v.EditorView.inputHandler.of((O, Q, e, a, i) => {
        if ((tO ? O.composing : O.compositionStarted) || O.state.readOnly || Q != e || (a != ">" && a != "/") || !B.isActiveAt(O.state, Q, -1))
          return false;
        let $ = i(),
          { state: t } = $;
        let r = t.changeByRange((O) => {
          var Q;
          let { head: e } = O,
            i = (0, d.syntaxTree)(t).resolveInner(e - 1, -1),
            $;
          if (i.name == "JSXStartTag") i = i.parent;
          if (t.doc.sliceString(e - 1, e) != a || (i.name == "JSXAttributeValue" && i.to > e));
            ...
            if (
              a &&
              O.from == e - 2 &&
              (($ = $O(t.doc, a.firstChild, e)) || ((Q = a.firstChild) === null || Q === void 0 ? void 0 : Q.name) == "JSXFragmentTag")
            ) {
              let O = `${$}>`;
            ...
            let Q = iO(i);
            if (Q && Q.name == "JSXOpenTag" && !/^\/?>|^<\//.test(t.doc.sliceString(e, e + 2)) && ($ = $O(t.doc, Q, e)))
              return { range: O, changes: { from: e, insert: `</${$}>` } };
          }
          return { range: O };
        });
      ...
      function SO(O, Q) {
        ...
        return (e) => {
          let { state: a } = e,
            i = [];
          for (let { from: $, to: t } of B.findRegions(a)) {
            let e = a.doc.lineAt($),
              r = { line: e.number - 1, col: $ - e.from, pos: $ };
          ...
          i.actions = [
            {
              name: "fix",
              apply(O, Q) {
                O.dispatch({ changes: { from: Q + t, to: Q + r, insert: $ }, scrollIntoView: true });
              },
            },
          ];
        }
