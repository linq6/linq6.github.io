      ...
      function i(e) {
        ...
        return {
          ...
          token: function (s, c) {
            ...
            var u;
            if (s.eatWhile(/\w/)) {
              if ((e && s.eat(".") && s.eatWhile(/\w/), (u = s.current()), !s.indentation())) return s.match(a) ? "number" : null;
              if ((1 == c.context || 4 == c.context) && i.test(u)) return (c.context = 4), "variable";
              if (2 == c.context && n.test(u)) return (c.context = 4), "variableName.special";
              if (t.test(u)) return (c.context = 1), "keyword";
              if (r.test(u)) return (c.context = 2), "keyword";
              if (4 == c.context && a.test(u)) return "number";
              if (l.test(u)) return "error";
            } else {
              if (s.eat(";")) return s.skipToEnd(), "comment";
              if (s.eat('"')) {
                for (; (u = s.next()) && '"' != u; ) "\\" == u && s.next();
                return "string";
              }
              if (s.eat("'")) {
