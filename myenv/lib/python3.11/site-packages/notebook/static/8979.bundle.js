  ...
  [8979],
  {
    38979: (E, T, I) => {
      ...
      const S = {
        ...
        token: function (E, T) {
          if ((null == T.indentStack && E.sol() && (T.indentation = 6), E.eatSpace())) return null;
          var I = null;
          if ("string" === T.mode) {
            ...
            var S = E.next(),
              U = E.column();
            if (U >= 0 && U <= 5) I = "def";
            else if (U >= 72 && U <= 79) E.skipToEnd(), (I = "header");
            else if ("*" == S && 6 == U) E.skipToEnd(), (I = "comment");
            else if ('"' == S || "'" == S) (T.mode = "string"), (I = N);
            else if ("'" != S || D.digit_or_colon.test(E.peek()))
              if ("." == S) I = "link";
              else if (
                (function (E, T) {
                  return "0" === E && T.eat(/x/i)
                    ? (T.eatWhile(D.hex), !0)
                    : (("+" != E && "-" != E) || !D.digit.test(T.peek()) || (T.eat(D.sign), (E = T.next())),
                      !!D.digit.test(E) &&
                        (T.eat(E),
                        T.eatWhile(D.digit),
                ...
                if (E.current().match(D.symbol)) for (; U < 71 && void 0 !== E.eat(D.symbol); ) U++;
                I =
                  C && C.propertyIsEnumerable(E.current().toUpperCase())
                    ? "keyword"
                    : L && L.propertyIsEnumerable(E.current().toUpperCase())
                      ? "builtin"
