    ...
    87674: (O, Q, $) => {
      ...
      const i = new e.Jq((O) => {
          ...
          for (; 40 != O.next; ) {
            ...
            (Q += String.fromCharCode(O.next)), O.advance();
          }
          for (O.advance(); ; ) {
            if (O.next < 0) return O.acceptToken(1);
            if (41 == O.next) {
              let $ = !0;
              for (let e = 0; $ && e < Q.length; e++) O.peek(e + 1) != Q.charCodeAt(e) && ($ = !1);
              if ($ && 34 == O.peek(Q.length + 1)) return O.acceptToken(1, 2 + Q.length);
            }
            O.advance();
          }
        }),
        a = new e.Jq(
          (O) => {
            if (62 == O.next) 62 == O.peek(1) && O.acceptToken(2, 1);
            else {
              let Q = !1,
                $ = 0;
              for (; ; $++) {
                if (O.next >= 65 && O.next <= 90) Q = !0;
                else {
                  if (O.next >= 97 && O.next <= 122) return;
                  if (95 != O.next && !(O.next >= 48 && O.next <= 57)) break;
                }
                O.advance();
              }
              Q && $ > 1 && O.acceptToken(3);
        ...
        r = (0, X.styleTags)({
          ...
          "extern MsCallModifier MsPointerModifier extern static register thread_local inline const volatile restrict _Atomic mutable constexpr constinit consteval virtual explicit VirtualSpecifier Access":
            X.tags.modifier,
          "if else switch for while do case default return break continue goto throw try catch": X.tags.controlKeyword,
          "co_return co_yield co_await": X.tags.controlKeyword,
          "new sizeof delete static_assert": X.tags.operatorKeyword,
          "NULL nullptr": X.tags.null,
          this: X.tags.self,
          "True False": X.tags.bool,
          "TypeSize PrimitiveType": X.tags.standard(X.tags.typeName),
          TypeIdentifier: X.tags.typeName,
          FieldIdentifier: X.tags.propertyName,
          "CallExpression/FieldExpression/FieldIdentifier": X.tags.function(X.tags.propertyName),
          "ModuleName/Identifier": X.tags.namespace,
          PartitionName: X.tags.labelName,
          StatementIdentifier: X.tags.labelName,
          "Identifier DestructorName": X.tags.variableName,
          "CallExpression/Identifier": X.tags.function(X.tags.variableName),
          "CallExpression/ScopedIdentifier/Identifier": X.tags.function(X.tags.variableName),
          "FunctionDeclarator/Identifier FunctionDeclarator/DestructorName": X.tags.function(X.tags.definition(X.tags.variableName)),
          NamespaceIdentifier: X.tags.namespace,
          OperatorName: X.tags.operator,
          ...
          AssignOp: X.tags.definitionOperator,
          UpdateOp: X.tags.updateOperator,
          LineComment: X.tags.lineComment,
          BlockComment: X.tags.blockComment,
          Number: X.tags.number,
          String: X.tags.string,
          "RawString SystemLibString": X.tags.special(X.tags.string),
          CharLiteral: X.tags.character,
          EscapeSequence: X.tags.escape,
          "UserDefinedLiteral/Identifier": X.tags.literal,
          PreProcArg: X.tags.meta,
          "PreprocDirectiveName #include #ifdef #ifndef #if #define #else #endif #elif": X.tags.processingInstruction,
          MacroName: X.tags.special(X.tags.name),
          "( )": X.tags.paren,
          "[ ]": X.tags.squareBracket,
          "{ }": X.tags.brace,
          "< >": X.tags.angleBracket,
          ". ->": X.tags.derefOperator,
          ", ;": X.tags.separator,
        }),
        P = {
          __proto__: null,
        ...
        U = { __proto__: null, "<": 131 },
        t = { __proto__: null, ">": 135 },
        Y = { __proto__: null, operator: 388, new: 576, delete: 582 },
        S = e.WQ.deserialize({
          version: 14,
          states:
          ...
          specialized: [
            { term: 356, get: (O) => P[O] || -1 },
            { term: 32, get: (O) => U[O] || -1 },
            { term: 66, get: (O) => t[O] || -1 },
            { term: 363, get: (O) => Y[O] || -1 },
          ],
          tokenPrec: 24891,
        });
      var l = $(17592);
      const n = l.LRLanguage.define({
        name: "cpp",
        parser: S.configure({
          props: [
            l.indentNodeProp.add({
              IfStatement: (0, l.continuedIndent)({ except: /^\s*({|else\b)/ }),
              TryStatement: (0, l.continuedIndent)({ except: /^\s*({|catch)\b/ }),
              LabeledStatement: l.flatIndent,
              CaseStatement: (O) => O.baseIndent + O.unit,
              BlockComment: () => null,
              CompoundStatement: (0, l.delimitedIndent)({ closing: "}" }),
              Statement: (0, l.continuedIndent)({ except: /^{/ }),
            }),
            l.foldNodeProp.add({
              "DeclarationList CompoundStatement EnumeratorList FieldDeclarationList InitializerList": l.foldInside,
      ...
      function s() {
        return new l.LanguageSupport(n);
      }
    },
  },
]);
